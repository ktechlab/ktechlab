...
//---------------------bank 0 registers return zero---------------------------//
    case TMR0:
        return Bank0& Bank1;//Bank0
    case PCL:
        return Bank0 & Bank1;//Bank0 | Bank1
    case STATUS:
        return Bank0 & Bank1;//Bank0 | Bank1
    case FSR:
        return Bank0 & Bank1;//Bank0 | Bank1
...
void RegisterState::print() {
    ...
    out << "   known=" << binary(known) << endl;
    out << "   value=" << binary(value) << endl;
}
...
void Instr_movf::generateLinksAndStates(Code::iterator current) {
    ...
    if (m_inputState.reg(m_file).known == 0xff) {
        ...
        if (isZero)
            m_outputState.status.value |= (1 << RegisterBit::Z);
        else    m_outputState.status.value &= ~(1 << RegisterBit::Z);
    } else  m_outputState.status.known &= ~(1 << RegisterBit::Z);
...
void Instr_subwf::generateLinksAndStates(Code::iterator current) {
    ...
    } else  m_outputState.reg(outputReg()).known = 0x0;
...
void Instr_btfsc::generateLinksAndStates(Code::iterator current) {
    ...
    } else  makeOutputLinks(current, true, true);
}
