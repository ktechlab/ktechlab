...
void Expression::traverseTree( BTreeNode *root, bool conditionalRoot)
{
	...
	if( t.current() == root && !root->hasChildren() &&
	...
	else	doOp( t.current()->childOp(), t.current()->left(), t.current()->right());
...
void Expression::doOp( Operation op, BTreeNode *left, BTreeNode *right)
{
	...
	if(left->reg().isEmpty())
		lvalue = left->value();
	else	lvalue = left->reg();
	
	QString rvalue;
	if(right->reg().isEmpty())
		rvalue = right->value();
	else	rvalue = right->reg();
