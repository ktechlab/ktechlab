...
//now INTCON.GIE is high ,and INTCON.GIE=1/high works
        if (isValidRegister( port))
        {
            ...
            holdport=Reg.name();
            if (holdport!=port)
                kError() << QString(" ERROR: %1 is not a Register bit\n").arg(portPinString );
        }
        else
...
//**************************Modification ends ********************************
    ...
    {
        kError() << QString("ERROR: %1 is not a Port/Register bit\n").arg(portPinString );
        return PortPin();
    }
}
...
//modified checking of 16F877 is included
    ...
    kError() << QString("%1 is not a known PIC identifier\n").arg(_text);
    return unknown;
}
...
void PIC14::add( QString val1, QString val2, LocationType val1Type, LocationType val2Type )
{
    ...
    {
    case num:
        m_pCode->append(new Instr_addlw(val2.toInt( 0, 0 )));
        break;
    case work:
        break;
    case var:
        m_pCode->append(new Instr_addwf(val2,0));
        break;
    }
}
...
void PIC14::subtract( const QString & val1, const QString & val2, LocationType val1Type, LocationType val2Type )
{
    ...
    {
    case num:
        m_pCode->append(new Instr_movlw( val2.toInt( 0, 0 ) ));
        break;
    case work:
        break;
    case var:
        m_pCode->append(new Instr_movf(val2,0));
        break;
    }
    switch (val1Type)
    {
    case num:
        m_pCode->append(new Instr_sublw(val1.toInt( 0, 0 )));
        break;
    case work:
        break;
    case var:
        m_pCode->append(new Instr_subwf(val1,0));
        break;
    }
}
...
void PIC14::bitwise( Expression::Operation op,const QString & r_val1, const QString & val2, LocationType val1Type, LocationType val2Type)
{
    ...
    case num:
    {
        ...
        case Expression::bwand:
            m_pCode->append(new Instr_andlw(val2.toInt( 0, 0 )));
            break;
        case Expression::bwor:
            m_pCode->append(new Instr_iorlw(val2.toInt( 0, 0 )));
            break;
        case Expression::bwxor:
            m_pCode->append(new Instr_xorlw(val2.toInt( 0, 0 )));
            break;
    ...
    case var:
    {
        ...
        {
        case Expression::bwand:
            m_pCode->append(new Instr_andwf(val2,0));
            break;
        case Expression::bwor:
            m_pCode->append(new Instr_iorwf(val2,0));
            break;
        case Expression::bwxor:
            m_pCode->append(new Instr_xorwf(val2,0));
            break;
        case Expression::bwnot:
            m_pCode->append(new Instr_xorwf(val2,0));
            break;
...
int PortPin::portPosition() const
{
    if ( m_port.isEmpty() )
        return 0;
    return uchar( m_port[ m_port.length() - 1 ].toAscii() ) - 'A';
}
