...
{
	if ( !sw || m_switchList.contains( sw ) ) return;
	m_switchList << sw;
}
...
{
	m_inputWireList.remove((Wire*)0 );
	m_outputWireList.remove((Wire*)0 );
	...
	for ( WireList::const_iterator it = inputs.begin(); it != end; ++it ) {
		if ( !(*it)->currentIsKnown() ) return false;
	...
	if(m_current >= 1e-8) {
		kdError() << "Pin current loss, magnitude: " << m_current << endl;
	}
