...
{
	if((m_angleDegrees%180) == 0) return;
...
void Button::setState( bool state )
{
	...
	if(isToggle()) m_button->setOn(state);
...
{
	if(isToggle()) return m_button->state();
...
void Slider::posChanged()
{
	...
	else m_slider->setOrientation( (m_angleDegrees%180 == 0) ? Qt::Horizontal : Qt::Vertical );
}
