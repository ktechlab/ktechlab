...
QString AsmFormatter::tidyInstruction( const QString & oldLine)
{
	...
	if(!parts.label().isEmpty()) line = parts.label() + ' ';
	pad( line, m_indentAsmName);
	
	if(!parts.operand().isEmpty()) line += parts.operand() + ' ';
	pad( line, m_indentAsmData);
	
	if(!parts.operandData().isEmpty()) line += parts.operandData();
...
AsmFormatter::LineType AsmFormatter::lineType( QString line)
{
	...
	for(QStringList::iterator it = parts.begin(); it != end; ++it) {
		if((*it).lower() == "equ") return Equ;
	}

	InstructionParts instructionParts(line);

	if(!instructionParts.operand().isEmpty()) return Instruction;
...
InstructionParts::InstructionParts(QString line)
{
	...
	for(QStringList::iterator it = parts.begin(); it != end; ++it) {
		if(foundOperand) {
			...
			// data.
			if(m_operandData.isEmpty()) m_operandData = *it;
			else m_operandData += ' ' + *it;
			
			continue;
		}
		
		if(PicAsm12bit::self()->operandList().contains((*it).upper()) || PicAsm14bit::self()->operandList().contains((*it).upper()) || PicAsm16bit::self()->operandList().contains((*it).upper())) {
			m_operand = *it;
			foundOperand = true;
		} else {
			// Must be a label
