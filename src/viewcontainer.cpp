...
View *ViewContainer::view( uint id ) const
{
    ...
    //find the child named View that is of type View
    View * view = va->findChild<View *>( "View" );
...
int ViewContainer::createViewArea( int relativeViewArea, ViewArea::Position position, bool showOpenButton )
{
    ...
    if (!relative)
    {
        kError() << "Could not find relative view area" << endl;
        return -1;
    }
...
void ViewContainer::updateCaption()
{
    ...
    KTabWidget *tab = KTechlab::self()->tabWidget();
    tab->setTabText( tab->indexOf( this ), caption );
}

void ViewContainer::updateWindowTitle()
{
    QString title;
    ...
    {
        Document *doc = activeView()->document();
        title = doc->url().isEmpty() ? doc->caption() : doc->url().fileName();
        if ( viewCount() > 1 )
            title += " ...";
    }
    ...
    KTabWidget *tab = KTechlab::self()->tabWidget();
    tab->setTabText( tab->indexOf( this ), title );
}
...
ViewArea *ViewArea::createViewArea( Position position, uint id, bool showOpenButton )
{
    ...
    {
        kError() << "Attempting to create ViewArea when already containing ViewAreas!" << endl;
        return 0l;
    }
    if (!p_view)
    {
        kError() << "We don't have a view yet, so creating a new ViewArea is redundant" << endl;
        return 0l;
    }

    setOrientation( ( position == Right ) ? Qt::Horizontal : Qt::Vertical );

    p_viewArea1 = new ViewArea( this, p_viewContainer, m_id, false, (const char*)("viewarea_"+QString::number(m_id).toUtf8()) );
    p_viewArea2 = new ViewArea( this, p_viewContainer, id, showOpenButton, (const char*)("viewarea_"+QString::number(id).toUtf8()) );
...
void ViewArea::setView( View *view )
{
    ...
    if ( p_view )
    {
        kError() << "Attempting to set already contained view!" << endl;
        return;
    }
...
/*
 * the QGridLayout should have 5 rows, 3 columns a margin of 0 and a spacing of 6
 */
