...
SourceFormatterController::SourceFormatterController(QObject *parent)
    : KDevelop::ISourceFormatterController(parent), m_modelinesEnabled(false)
{
}
...
KDevelop::IPlugin* SourceFormatterController::languageSupportForMimeType(const QString &name)
{
    KDevelop::IPluginController *controller = KDevelop::ICore::self()->pluginController();
    ...
    // FIXME: is this correct? Doesn't return return on the first result? Is this intended?
...
KDevelop::ISourceFormatter* SourceFormatterController::activeFormatter()
{
    return m_currentPlugins[m_currentLang];
}

KDevelop::ISourceFormatter* SourceFormatterController::formatterForUrl(const KUrl &url)
{
    KMimeType::Ptr mime = KMimeType::findByUrl(url);
    return formatterForMimeType(mime);
}

KDevelop::ISourceFormatter* SourceFormatterController::formatterForMimeType(const KMimeType::Ptr &mime)
{
...
KDevelop::ISourceFormatter* SourceFormatterController::formatterForLanguage(const QString &language)
{
...
KDevelop::ISourceFormatter* SourceFormatterController::formatterByName(const QString &language, const QString &name)
{
...
void SourceFormatterController::setActiveLanguage(const QString &lang, QString plugin)
{
    ...
    if (plugin.isEmpty()) {
        if (m_currentLang == lang)
            return; // no change
        KDevelop::ISourceFormatter *f = formatterForLanguage(lang);
        if (f)
            plugin = f->name();
    } else
...
QString SourceFormatterController::addModelineForCurrentLang(QString input, const KMimeType::Ptr &mime)
{
    ...
    KDevelop::ISourceFormatter::IndentationType type = m_currentPlugins[m_currentLang]->indentationType();
    if (type == KDevelop::ISourceFormatter::IndentWithTabs) {
        ...
        if (type == KDevelop::ISourceFormatter::IndentWithSpacesAndConvertTabs)
