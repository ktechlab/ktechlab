    ...
    , p_circuitDocument(circuitDocument)
{
    ...
    setWhatsThis(
        i18n("Construct a circuit by dragging components from the Component selector from the left. Create the connections by dragging a wire from the component connectors.<br><br>"
...
void CircuitView::dragEnterEvent(QDragEnterEvent *e)
{
    ...
    bool acceptable = e->mimeData()->hasFormat("ktechlab/component") || e->mimeData()->hasFormat("ktechlab/subcircuit");
    if (!acceptable)
        return;
