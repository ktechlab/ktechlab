...
private:
     ...
     */
    Connector* findCommonConnector(Node *n1, Node *n2) const;
  ...
  /**
     ...
     */
    void findBestPair(NodeList *list, Node **n1, Node **n2) const;
     ...
     */
    bool canRoute(Node *n1, Node *n2) const;
    void getReachable(IntList *reachable, int node) const;
    /**
     * Either: position of node in m_nodeList,
     * or: (position of node in m_extNodeList) + m_nodeList.size()
     * or: -1
     */
    int getNodePos(Node *n) const;
    /**
     * Essentially the inverse of getNodePos
     */
    Node* getNodePtr(unsigned int n) const;
