...
{
    return new LibraryItem(QStringList(QString("flow/forloop")), i18n("For"), i18n("Loops"), "for.png", LibraryItem::lit_flowpart, ForLoop::construct);
}
...
void ForLoop::dataChanged()
{
    if (dataString("3-step").toInt() == 1)
        ...
        setCaption("for " + dataString("0-var") + " = " + dataString("1-initial") + " to " + dataString("2-end") + " step " + dataString("3-step"));
}

void ForLoop::generateMicrobe(FlowCode *code)
{
    if (dataString("3-step").toInt() == 1)
        code->addCode("for " + dataString("0-var") + " = " + dataString("1-initial") + " to " + dataString("2-end") + "\n{");
    else
        code->addCode("for " + dataString("0-var") + " = " + dataString("1-initial") + " to " + dataString("2-end") + " step " + dataString("3-step") + "\n{");
