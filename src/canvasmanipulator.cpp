...
/* TODO AUDIT CODE
Formatting was very ugly. Probably buggy and/or poorly factored. 
*/ 
	...
	: QObject()
{
	...
	p_lastMouseOverResizeHandle = 0;
	m_canvasManipulator = 0;
	p_itemDocument = itemDocument;
	m_cmState = 0;
	p_lastMouseOverItem = 0;
	p_lastItemClicked = 0;
	m_drawAction = -1;
...
void CMManager::mouseMoveEvent( const EventInfo &eventInfo)
{
	if(m_canvasManipulator)
	{
		...
		if(itemView) itemView->scrollToMouse(eventInfo.pos);
		return;
	}
	...
	if( p_lastMouseOverItem != (QGuardedPtr<Item>)item) {
		...
		if(p_lastMouseOverItem) p_lastMouseOverItem->leaveEvent();
...
void CMManager::updateCurrentResizeHandle( ResizeHandle * resizeHandle)
{
	if( p_lastMouseOverResizeHandle != (QGuardedPtr<ResizeHandle>)resizeHandle)
	{
		...
		if(resizeHandle) resizeHandle->setHover(true);
	}
}
...
{
	if( m_drawAction == drawAction) return;
...
{
	if(!clickedConnector) return clickPos;
	...
	for( unsigned i = 0; i < 5; ++i) {
		for( QPointList::const_iterator it = pointList.begin(); it != end; ++it) {
			if(qpoint_distance( *it, clickPos) <= dl[i]) return *it;
		}
	}
...
{
	if( !node || node->numCon( true, false) < 3) return 0;
...
bool CMManualConnector::mouseMoved( const EventInfo &eventInfo)
{
	...
	if( movedFlag) {
		QCanvasItem *startItem = 0;
		if(p_startNode) startItem = p_startNode;
		else if(p_startConnector)
			startItem = p_startConnector;
...
bool CMManualConnector::mouseReleased( const EventInfo &eventInfo)
{
	...
	if(!p_endNode && !p_endConnector) return false;
...
bool CMItemMove::mousePressedInitial( const EventInfo &eventInfo)
{
	...
	if( p_selectList->isEmpty()) return true;
...
bool CMItemMove::mouseMoved( const EventInfo &eventInfo)
{
	...
	for( ItemList::const_iterator it = itemList.begin(); it != end; ++it)
	{
		...
		if(CNItem *cnItem = dynamic_cast<CNItem*>((Item*)*it)) {
			...
			dx += int((*it)->x());
			dy += int((*it)->y());
		} else (*it)->moveBy( eventInfo.pos.x()-m_prevPos.x(), eventInfo.pos.y()-m_prevPos.y());
...
bool CMDraw::mouseMoved( const EventInfo &eventInfo)
{
	...
	if(m_pDrawRectangle) m_pDrawRectangle->setSize(pos.x()-m_eventInfo.pos.x(), pos.y()-m_eventInfo.pos.y());
...
{
	if(b_orientationDefined) b_currentVertical = !b_currentVertical;
	else mouseMoved(pos);
