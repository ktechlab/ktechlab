...
void FlowCode::processInput(ProcessOptions options)
{
    ...
    if (!options.p_flowCodeDocument) {
        options.p_flowCodeDocument = new FlowCodeDocument(QString());
...
void FlowCode::tidyCode()
{
    ...
    while (++pos < length) {
        ...
        case '\n': {
            ...
            newCode += '\n';
            if (!comment && !asmEmbed) {
                while (pos + 1 < length && m_code[pos + 1].isSpace())
                    pos++;
        ...
        case '/': {
            ...
            if (pos + 1 < length) {
                if (m_code[pos + 1] == '/')
                    comment = true;
                else if (m_code[pos + 1] == '*')
                    multiLineComment = comment = true;
                newCode += m_code[++pos];
            }
        ...
        case '*': {
            newCode += '*';
            if (pos + 1 < length) {
                if (m_code[pos++] == '/' && multiLineComment)
                    comment = multiLineComment = false;
                newCode += m_code[pos];
            }
            asmEmbedAllowed = false;
            asmKeyword = false;
...
{
    if (!part || m_subroutines.contains(part) || part->parentItem() || !dynamic_cast<FlowContainer *>(part))
        return;
