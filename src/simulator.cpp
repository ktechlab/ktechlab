...
Simulator::~Simulator()
{
	...
	unsigned max = unsigned(MAX_CALLBACKS);
	for( unsigned i = 0; i < max; i++) {
		if(m_pStartStepCallback[i]) {
			delete m_pStartStepCallback[i];
			m_pStartStepCallback[i] = 0;
		}
	}
}
...
void Simulator::step()
{
	...
	for( unsigned i = 0; i < maxSteps; ++i) {
		...
		const unsigned max = unsigned(MAX_CALLBACKS);
...
{
// TODO: MEMORY LEAK CHECK, I don't think I did this right. 
...
{
	if(!component || !component->doesStepNonLogic()) return;
...
class pred1_c {
	public:
	pred1_c(const Component &comp) : component(&comp) {};
	bool pred1(ComponentCallback *x) { return x->component() == component; }
private :
...
void Simulator::removeLogicOutReferences(LogicOut *logic)
{
	...
	if(m_pChangedLogicLast == logic) {
		...
		for(LogicOut *logic = m_pChangedLogicStart; logic;) {
			if(previous_1) previous_2 = previous_1;
			previous_1 = logic;
