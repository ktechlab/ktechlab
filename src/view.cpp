...
View::View( Document *document, ViewContainer *viewContainer, uint viewAreaId, const char *name )
    : KXmlGuiWindow( viewContainer->viewArea( viewAreaId ) )
{
    name = name ? name : (const char *)( "view_" + QString::number(viewAreaId).toUtf8() );
    setObjectName( name );
...
//     if ( !action )
//         kError() << "No such action: " << name << endl;
//     return action;
// }
...
// 			kdDebug() << k_funcinfo << "Focused Out.\n";
            ...
            if ( QWidget * fw = KTechlab::self()->focusWidget() )
            {
                QString fwClassName( fw->objectName() );
...
{
    m_fileNameLabel->setText( url.isEmpty() ? i18n("Untitled") : url.fileName() );
}
