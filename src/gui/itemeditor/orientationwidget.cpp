...
OrientationWidget::OrientationWidget(QWidget *parent, const char *name)
        : QWidget(parent, name), m_pCNItem(0) {
...
void OrientationWidget::initFromComponent(Component * component) {
    ...
    for (unsigned col = 0; col < 4; ++col) {
        for (unsigned row = 0; row < 2; ++row) {
            ...
            if (col || row)
                 tbPm.convertFromImage(
			im.xForm(Component::transMatrix(
				angle, flipped, bound.width() / 2, bound.height() / 2)));
            else tbPm.convertFromImage(im);
...
void OrientationWidget::slotButtonClicked() {
    ...
    for (int row = 0; row < 2; ++row) {
        for (int col = 0; col < 4; ++col) {
            ...
            else ItemInterface::self()->setComponentOrientation(90*col, bool(row));
        }
    }
}
