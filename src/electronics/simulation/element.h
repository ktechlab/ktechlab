...
//	CBranch *cbranch( const uint32_t num) { return p_cbranch[num]; }
	 ...
	 */
	virtual Type type() const { return Element_abstract; }
	/**
...
// what is this actually used for? is it necessary?	
	...
	bool getStatus() {
		if(status_changed) updateStatus();
		return b_status; }
...
{
	if(cnodes[i]==-1 || cnodes[j]==-1) return m_temp;
	return p_eSet->matrix()->g(cnodes[i], cnodes[j]);
}

double & Element::A_b(uint32_t i, uint32_t j)
{
	if(cnodes[i]==-1) return m_temp;
	return p_eSet->matrix()->b(cnodes[i], cbranches[j]);
}

double & Element::A_c(uint32_t i, uint32_t j)
{
	if(cnodes[j]==-1) return m_temp;
	return p_eSet->matrix()->c(cbranches[i], cnodes[j]);
}

double & Element::A_d(uint32_t i, uint32_t j)
{
	return p_eSet->matrix()->d(cbranches[i], cbranches[j]);
}

double & Element::b_i(uint32_t i)
{
	if(cnodes[i]==-1) return m_temp;
	return (*p_eSet->b())[cnodes[i]];
}

double & Element::b_v(uint32_t i)  {
	return (*p_eSet->b())[p_eSet->cnodeCount() + cbranches[i]];
}
