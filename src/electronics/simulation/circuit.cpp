...
void Circuit::init()
{
	...
	for(ElementList::iterator it = m_elementList.begin(); it != listEnd; ++it)
	{
		...
		switch((*it)->numCBranches()) {
			...
			case 1:
				(*it)->setCBranches(i);
//if we try to get too slick and put the ++'s inside the ()'s, the compiler starts
// getting nervous, (eg   foo(i++,i++);,) so let's play it safe. 
				i++;
				break;
			case 2:
...
// note: there are three classes of elements, LOGIC, (and gates, etc) LINEAR
//  (resistors, etc, and NON-LINEAR (diodes and stuff.).
...
void Circuit::updateCurrents()
{
	...
	if(error>1e-9) {
// you'll see this alot because ECground isn't implemented right.
...
// there are also some "magical" components such as op-amps which contain hidden
// voltage points within them. 
