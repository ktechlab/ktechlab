...
*/
	 ...
	 */
	QuickVector *fbSub(const QuickVector *x) const;
...
//	const double g( uint i, uint j) const { return (*m_mat)[m_inMap[i]][j]; }
	...
	const double g( uint i, uint j) const { return (*m_mat)[i][j]; }
...
//	int *m_inMap; // Rowwise permutation mapping from external reference to internal storage
	...
	QuickMatrix *P; // -- permutation matrix; quite inefficient

//	Map *m_map; // FIXME: not used with current LU engine,
};
