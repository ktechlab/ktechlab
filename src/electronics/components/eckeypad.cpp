...
{
    return new LibraryItem(QStringList(QString("ec/keypad")), i18n("Keypad"), i18n("Switches"), "keypad.png", LibraryItem::lit_component, ECKeyPad::construct);
}

const QString text[4][9] = {{"1", "2", "3", "A", "E", "I", "M", "Q", "U"}, {"4", "5", "6", "B", "F", "J", "N", "R", "V"}, {"7", "8", "9", "C", "G", "K", "O", "S", "W"}, {"*", "0", "#", "D", "H", "L", "P", "T", "X"}};
...
void ECKeyPad::initPins(unsigned numCols)
{
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < int(m_numCols); j++) {
            widgetWithID(buttonID(i, j))->setOriginalRect(QRect(offsetX() + 6 + 24 * j, offsetY() + 6 + 24 * i, 20, 20));
        }
    }
...
void ECKeyPad::buttonStateChanged(const QString &id, bool state)
{
    ...
    QStringList tags = id.split('_', Qt::SkipEmptyParts); // QStringList::split( '_', id ); // 2018.12.01
