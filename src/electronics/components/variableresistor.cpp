...
{
    return new LibraryItem(QStringList(QString("ec/variableresistor")), i18n("Variable Resistor"), i18n("Passive"), "variable_resistor.png", LibraryItem::lit_component, VariableResistor::construct);
}
...
void VariableResistor::dataChanged()
{
    ...
    if (new_minResistance != m_minResistance) {
        if (new_minResistance >= m_maxResistance) {
            ...
            m_minResistance = new_minResistance;
    }
    ...
    if (new_maxResistance != m_maxResistance) {
        if (new_maxResistance <= m_minResistance) {
            ...
            m_maxResistance = new_maxResistance;
    }
...
void VariableResistor::sliderValueChanged(const QString &id, int newValue)
{
    ...
    QString display = QString::number(m_currResistance / getMultiplier(m_currResistance), 'g', 3) + getNumberMag(m_currResistance) + QChar(0x3a9);
