...
QuickVector *QuickMatrix::transposeMult(const QuickVector *operandvec) const {
	...
	for(unsigned int i = 0; i < n; i++) {
		...
		for(unsigned int j = 0; j < m; j++)
			sum += values[j][i] * operandvec->at(j+1);
		ret->atPut(i+1,sum);
	}
...
{
//	if(!m_a || !n_a || m_a > m || n_a > n) return NAN;

	return values[m_a - 1][n_a - 1];
}
...
{
	const unsigned int posfix = pos - 1; 
	return values[row - 1][posfix] * values[posfix][col - 1];
}
...
{
	CUI rlim = lim - 1;
	const double *rowVals = values[row - 1];
	CUI rcol = col - 1;

	double sum = 0;
 	for(unsigned int i = 0; i < rlim; i++)
		sum += rowVals[i] * values[i][rcol];
	return sum;
}
...
bool QuickMatrix::atPut(CUI m_a, CUI n_a, const double val) {
	if(!m_a || !n_a || m_a > m || n_a > n) return false;

	values[m_a - 1][n_a - 1] = val;
	return true;
}
...
bool QuickMatrix::scaleAndAdd(CUI m_a, CUI m_b, const double scalor) {
	if(!m_a || !m_b || m_a > m || m_b > m) return false;

	const double *arow = values[m_a - 1];
	double *brow = values[m_b - 1];
...
// iterate over n - m_a columns.
	for(unsigned int j = m_a - 1; j < n; j++)
		brow[j] += arow[j] * scalor;
...
QuickVector *QuickMatrix::operator *(const QuickVector *operandvec) const {
	...
	for(unsigned int i = 0; i < m; i++) {
		double sum = 0;
		for(unsigned int j = 0; j < n; j++)
			sum += values[i][j] * operandvec->at(j+1);
		ret->atPut(i+1,sum);
	}
